...
function Moviecard({ title, genre, year, rating, image, favorite = false }) {
  ...
  return (
    <div className="movie-card w-48 group">
      {/* Container Poster */}
      <figure className="movie-poster relative bg-transparent rounded-2xl overflow-hidden aspect-2/3 shadow-xl border border-white/10">
        {image ? (
          <img
            src={image}
            alt={title}
        ...
        <div className="category-favorite flex items-center justify-between gap-x-2 absolute top-0 left-0 w-full p-3  from-black/70 to-transparent">
          ...
          <button
            ...
            className="bg-transparent backdrop-blur-sm w-8 h-8 flex items-center justify-center rounded-full cursor-pointer  hover:scale-110 transition duration-200 active:scale-95 border border-white/10"
      ...
      <div className="movie-title flex justify-between items-start mt-3 px-1">
        ...
        {/* Rating */}
        <div className="rating flex items-center gap-x-1 shrink-0 mt-0.5 px-1 py-0.5 bg-black/40 backdrop-blur-sm rounded-md">
